...
class IndexingWorker:
    def __init__(self, repository, queue, embeddings, worker_id: str | None = None):
    ...
    async def process(self, message_id: str, fields: dict) -> None:
        ...
        if claimed is None:
            if await asyncio.to_thread(self.repository.fail_exhausted, contract.run_id):
        ...
        try:
            ...
            while not self.stop_event.is_set():
                ...
                if not rows:
                    break
                vectors = await self.embeddings.embed([row[1].content for row in rows])
                await asyncio.to_thread(self.repository.persist_batch, run.id, token, rows, vectors)
                after_id = rows[-1][0].id
            if self.stop_event.is_set():
                return
            if await asyncio.to_thread(self.repository.complete, run.id, token):
            ...
            if await asyncio.to_thread(
                ...
                if await asyncio.to_thread(self.repository.fail_exhausted, run.id):
                    await self.queue.dlq(message_id, contract, "retry_exhausted")
                    await self.queue.ack(message_id)
            ...
            cancelled = await asyncio.to_thread(self.repository.finish_cancelled, run.id, token)
            if cancelled:
                await self.queue.ack(message_id)
            elif await asyncio.to_thread(self.repository.fail, run.id, token, "indexing_failed"):
