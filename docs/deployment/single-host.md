...
4. Copy `.env.production.example` to a root-readable secret file outside the checkout. Replace every `change-me` and `.invalid`. Prefer Docker secrets for passwords/tokens.
...
docker compose --env-file /run/secrets/rag.env run --rm migrate
docker compose --env-file /run/secrets/rag.env up -d embedding-service api inference-worker ingestion-worker indexing-worker scheduler web
docker compose --env-file /run/secrets/rag.env ps
```

If PostgreSQL/Redis/models are external, use a private override file that removes local services and injects TLS URLs. Bootstrap admin without putting the password on the command line:
