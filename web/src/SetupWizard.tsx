...
export function SetupWizard({ initial, principal, onAuthenticated, onComplete }: Props) {
  ...
  async function createAdministrator(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const authenticated = await api.bootstrapSetup({
        tenant_slug: 'local', tenant_name: String(data.get('tenant_name')),
  ...
  return <main className="setup-shell"><section className="setup-card card">
    ...
    {step === 'administrator' && <form className="form-grid" onSubmit={createAdministrator}><h2>Рабочее пространство и администратор</h2><label>Название пространства<input name="tenant_name" defaultValue="Моё пространство" required /></label><label>Логин администратора<input name="username" autoComplete="username" defaultValue="admin" required /></label><label>Имя администратора<input name="display_name" autoComplete="name" required /></label><label>Пароль<input name="password" type="password" minLength={12} autoComplete="new-password" required /></label><label>Повторите пароль<input name="confirmation" type="password" minLength={12} autoComplete="new-password" required /></label>{status.token_required && <label>Bootstrap token<input name="setup_token" type="password" required /></label>}<button disabled={busy}>{busy ? 'Создаём…' : 'Создать защищённое пространство'}</button></form>}
    {step === 'models' && <form className="form-grid" onSubmit={configureModels}><h2>Self-hosted модели</h2><fieldset><legend>Generation</legend><label>Endpoint<input name="generation_url" type="url" defaultValue="http://host.docker.internal:11434/v1" required /></label><label>Model ID<input name="generation_model" defaultValue="qwen2.5-coder:14b" required /></label><label>Version<input name="generation_version" defaultValue="qwen2.5-coder/14b" required /></label><label>Bearer token (необязательно)<input name="generation_token" type="password" autoComplete="off" /></label></fieldset><fieldset><legend>Embedding</legend><label>Endpoint<input name="embedding_url" type="url" defaultValue="http://embedding-service:8001/v1" required /></label><label>Model ID<input name="embedding_model" defaultValue="BAAI/bge-m3" required /></label><label>Version<input name="embedding_version" defaultValue="bge-m3/1" required /></label><label>Dimensions<input name="embedding_dimensions" type="number" defaultValue="1024" min="1" required /></label></fieldset><fieldset><legend>Reranker (необязательно)</legend><label>Endpoint<input name="reranker_url" type="url" /></label><label>Model ID<input name="reranker_model" /></label><label>Version<input name="reranker_version" defaultValue="1" /></label><label>Bearer token<input name="reranker_token" type="password" autoComplete="off" /></label></fieldset><button disabled={busy}>{busy ? 'Проверяем…' : 'Сохранить и проверить модели'}</button></form>}
